...
import { ArrowLeft, Camera, Calendar, Image as ImageIcon, Wifi, Car, Coffee, Music, Snowflake, Tv, Accessibility, Wine, Rocket, Lock, Loader2, Sparkles, MapPin, Wand2 } from 'lucide-react';
...
const CreateEventPage = () => {
  ...
  return (
    <div className="pb-24 lg:pb-0 lg:flex lg:h-screen lg:overflow-hidden">
      {/* Left: Form */}
      <div className="px-5 pt-safe lg:w-1/2 lg:overflow-y-auto lg:p-8">
        <div className="pt-6 mb-6 flex items-center gap-3">
          <button onClick={goBack} className="lg:hidden w-10 h-10 rounded-xl bg-card border border-border flex items-center justify-center">
        ...
        <div className="space-y-4 max-w-2xl mx-auto">
          ...
          <div>
            ...
            >
              {form.image ? (
                <>
                  <img src={form.image} loading="lazy" alt="Preview" className="w-full h-full object-cover" />
            ...
            <div className="flex gap-2 overflow-x-auto pb-2">
              {form.extraImages.map((img, idx) => (
                <div key={`extra-img-${idx}`} className="relative w-20 h-20 flex-shrink-0 rounded-xl overflow-hidden border border-border group">
                  <img src={img} loading="lazy" alt={`Extra ${idx}`} className="w-full h-full object-cover" />
                  <button 
      ...
      {/* Right: Live Preview */}
      <div className="hidden lg:flex lg:w-1/2 bg-muted/30 overflow-y-auto p-8 items-start justify-center">
        <div className="sticky top-8 w-full max-w-md">
          <div className="flex items-center gap-2 mb-6">
            <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse" />
            <span className="text-[10px] font-black uppercase tracking-[0.25em] text-muted-foreground">Vista previa en vivo</span>
          </div>
          ...
          <div className="bg-card rounded-3xl border border-border overflow-hidden shadow-xl shadow-black/5">
              ...
              {form.image ? (
                ...
                <div className="w-full h-full flex items-center justify-center">
                  <div className="text-center">
                    ...
                    <p className="text-xs text-muted-foreground/50 font-medium">Sin portada</p>
                  </div>
                </div>
              )}
              <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent" />
              {form.price && parseFloat(form.price) > 0 && (
                <div className="absolute top-4 right-4 bg-foreground text-background px-3 py-1.5 rounded-xl text-sm font-black shadow-lg">
                  {form.currency === 'USD' ? '$' : 'COL$'}{form.price}
                </div>
              )}
              {(!form.price || parseFloat(form.price) === 0) && (
                <div className="absolute top-4 right-4 bg-green-500 text-white px-3 py-1.5 rounded-xl text-sm font-black shadow-lg">
                  Gratis
                </div>
              )}
            </div>
            ...
            <div className="p-6 space-y-5">
              ...
              {form.categories.length > 0 && (
                <div className="flex flex-wrap gap-1.5">
                  {form.categories.map(cat => (
                    <span key={cat} className="px-2.5 py-1 bg-muted rounded-lg text-[10px] font-bold uppercase tracking-wider">
                      {categoryEmojis[cat]} {cat}
                    </span>
                  ))}
                </div>
              ...
              <h2 className="text-2xl font-black text-foreground tracking-tight leading-tight">
                {form.title || <span className="text-muted-foreground/40">Título del evento</span>}
              </h2>
                ...
                {(form.date || form.time) && (
                  <div className="flex items-center gap-3 text-sm text-muted-foreground">
                    ...
                    <span className="font-medium">
                      {form.date ? new Date(form.date).toLocaleDateString('es-ES', { weekday: 'long', day: 'numeric', month: 'long' }) : 'Sin fecha'}
                      {form.time && ` — ${form.time}`}
                    </span>
                  </div>
                )}
              ...
              {form.amenities.length > 0 && (
                <div>
                  <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-2">Servicios incluidos</p>
                  <div className="flex flex-wrap gap-2">
                    {AVAILABLE_AMENITIES.filter(a => form.amenities.includes(a.id)).map(a => {
                      const Icon = a.icon;
                      return (
                        <div key={a.id} className="flex items-center gap-1.5 px-3 py-1.5 bg-muted rounded-lg text-xs font-medium">
              ...
              <button
                onClick={handleSubmit}
                disabled={isSubmitting}
                className="w-full bg-foreground text-background py-4 rounded-2xl font-black text-sm uppercase tracking-widest shadow-lg active:scale-[0.98] transition-all disabled:opacity-50 flex items-center justify-center gap-2 mt-2"
              >
                {isSubmitting ? (
                  <>
                    <Loader2 className="w-5 h-5 animate-spin" />
          ...
          <p className="text-center text-[10px] text-muted-foreground/50 mt-4 font-medium">
