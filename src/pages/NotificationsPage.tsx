...
const NotificationsPage = () => {
  ...
  const markAllReadMutation = useMutation({
    ...
    onMutate: async () => {
      ...
      const prev = queryClient.getQueryData<any[]>(queryKeys.notifications.all);
      queryClient.setQueryData(queryKeys.notifications.all, (old: any[] = []) => old.map(n => ({ ...n, read: true })));
      return { prev };
    },
  ...
  return (
    <div className="min-h-screen bg-background pb-24 lg:pb-8">
      <div className="sticky top-0 z-10 bg-background/80 backdrop-blur-md border-b border-border p-4 flex items-center justify-between">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-56 rounded-2xl p-2 shadow-xl border-border bg-card">
            <DropdownMenuItem onClick={() => markAllReadMutation.mutate()} className="gap-3 px-4 py-3 rounded-xl cursor-pointer font-bold text-[13px] text-foreground hover:bg-secondary">
              <CheckCheck className="w-4 h-4 text-emerald-500" /> Marcar como leídas
            </DropdownMenuItem>
            <DropdownMenuSeparator className="my-1 bg-border" />
            <DropdownMenuItem onClick={() => clearAllMutation.mutate()} className="gap-3 px-4 py-3 rounded-xl cursor-pointer font-bold text-[13px] text-rose-500 hover:bg-rose-500/10">
      ...
      <div className="divide-y divide-border">
        ...
        )) : <EmptyState t={{}} navigate={navigate} />}
      </div>
    </div>
  );
};

const NotificationItem = ({ notif, onClick }: { notif: any; onClick: (n: any) => void }) => {
  const icon = {
    event: <Star className="w-4 h-4 text-amber-500" />,
    ticket: <Ticket className="w-4 h-4 text-green-500" />,
    chat: <MessageSquare className="w-4 h-4 text-blue-500" />,
    follow: <UserPlus className="w-4 h-4 text-purple-500" />,
  }[notif.type] || <Info className="w-4 h-4 text-primary" />;

  return (
    <div onClick={() => onClick(notif)} className={`p-5 flex gap-4 transition-colors hover:bg-secondary/30 cursor-pointer ${!notif.read ? 'bg-primary/5' : ''}`}>
      ...
      <div className="flex-1 space-y-1">
        <div className="flex justify-between items-start gap-2">
          <h3 className={`text-sm font-bold leading-tight ${!notif.read ? 'text-foreground' : 'text-foreground/80'}`}>{notif.title}</h3>
...
const EmptyState = ({ navigate }: { navigate: any }) => (
  <div className="flex flex-col items-center justify-center py-32 px-10 text-center">
    <div className="relative mb-8">
      <div className="relative w-24 h-24 bg-gradient-to-br from-amber-400 via-orange-500 to-rose-500 rounded-[32px] shadow-2xl shadow-orange-500/40 flex items-center justify-center">
        <Bell className="w-12 h-12 text-white" />
        <Sparkles className="absolute -top-2 -right-2 w-8 h-8 text-amber-300" />
      </div>
    </div>
    ...
    <Button onClick={() => navigate('/')} className="w-full h-14 rounded-2xl bg-foreground text-background font-black text-sm uppercase tracking-widest shadow-xl shadow-foreground/10 active:scale-95 transition-all hover:opacity-90 border-none">
      Explorar Eventos
    </Button>
  </div>
);
